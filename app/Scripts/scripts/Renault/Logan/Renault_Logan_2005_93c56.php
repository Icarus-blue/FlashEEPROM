<?php

namespace App\Scripts;

class Renault_Logan_2005_93c56 extends Script
{
    public function getResult()
    {
        $hex = $this->getByteForPosition('D0',8). $this->getByteForPosition('D0', 9) . $this->getByteForPosition('D0',6). $this->getByteForPosition('D0',7);
        $number = hexdec($hex);
         
        return [
            'result' => round($number),
            'image' => 'assets/Renault.png',
            'texts' => [
                'Logan 2005 ',
                'Eeprom 93c56',
                'www.FlashEeprom.com'
            ],
            'fileprefix' => 'flasheeprom'
        ];
    }
    
    public function calculate(int $value)
    {   $result = round(($value));
        $hex = str_pad(dechex($result), 8, '0', STR_PAD_LEFT);

        $result2 = round(4294967295-($value));
        $hex2 = str_pad(dechex($result2), 8, '0', STR_PAD_LEFT);

        
        return
         [
            ['row' => 'D0', 'cell' => 4, 'value' => substr($hex2, 4, 2)],
            ['row' => 'D0', 'cell' => 5, 'value' => substr($hex2, 6, 2)],
            ['row' => 'D0', 'cell' => 6, 'value' => substr($hex, 4, 2)],
            ['row' => 'D0', 'cell' => 7, 'value' => substr($hex, 6, 2)],
            ['row' => 'D0', 'cell' => 8, 'value' => substr($hex, 0, 2)],
            ['row' => 'D0', 'cell' => 9, 'value' => substr($hex, 2, 2)],

            ['row' => 'D0', 'cell' => 10, 'value' => substr($hex2, 4, 2)],
            ['row' => 'D0', 'cell' => 11, 'value' => substr($hex2, 6, 2)],
            ['row' => 'D0', 'cell' => 12, 'value' => substr($hex, 4, 2)],
            ['row' => 'D0', 'cell' => 13, 'value' => substr($hex, 6, 2)],
            ['row' => 'D0', 'cell' => 14, 'value' => substr($hex, 0, 2)],
            ['row' => 'D0', 'cell' => 15, 'value' => substr($hex, 2, 2)],

            ['row' => 'E0', 'cell' => 0, 'value' => substr($hex2, 4, 2)],
            ['row' => 'E0', 'cell' => 1, 'value' => substr($hex2, 6, 2)],
            ['row' => 'E0', 'cell' => 2, 'value' => substr($hex, 4, 2)],
            ['row' => 'E0', 'cell' => 3, 'value' => substr($hex, 6, 2)],
            ['row' => 'E0', 'cell' => 4, 'value' => substr($hex, 0, 2)],
            ['row' => 'E0', 'cell' => 5, 'value' => substr($hex, 2, 2)],

            ['row' => 'E0', 'cell' => 6, 'value' => substr($hex2, 4, 2)],
            ['row' => 'E0', 'cell' => 7, 'value' => substr($hex2, 6, 2)],
            ['row' => 'E0', 'cell' => 8, 'value' => substr($hex, 4, 2)],
            ['row' => 'E0', 'cell' => 9, 'value' => substr($hex, 6, 2)],
            ['row' => 'E0', 'cell' => 10, 'value' => substr($hex, 0, 2)],
            ['row' => 'E0', 'cell' => 11, 'value' => substr($hex, 2, 2)],
         

            
        ];
    }
}
