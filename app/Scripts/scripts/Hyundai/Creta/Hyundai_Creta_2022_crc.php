...
class Hyundai_Creta_2022_crc extends Script
{
    ...
    { $byte = $this->getByteForPosition('920', 1);
        
        $a = substr($byte, 0, 1);
        $b =substr($byte, -1);

        $byte2=$this->getByteForPosition('920', 0);
        $c = substr($byte2, 0, 1);
        $hex = $a . $b. $c ;
        $number = hexdec($hex); 
    ...
    private function calculateRepeatedValue(int $value) : array
    {
        ...
        return
         [
            ...
            ['row' => '920', 'cell' => 0, 'value' => substr($hex9, 0, 2)],
            ['row' => '920', 'cell' => 1, 'value' => substr($hex2, 0, 2)], 
            ['row' => '920', 'cell' => 4, 'value' => substr($hex9, 0, 2)],
            ['row' => '920', 'cell' => 5, 'value' => substr($hex2, 0, 2)], 
            ['row' => '920', 'cell' => 8, 'value' => substr($hex9, 0, 2)],
            ['row' => '920', 'cell' => 9, 'value' => substr($hex2, 0, 2)], 
            ['row' => '920', 'cell' => 12, 'value' => substr($hex9, 0, 2)],
            ['row' => '920', 'cell' => 13, 'value' => substr($hex2, 0, 2)], 
            
            ['row' => '930', 'cell' => 0, 'value' => substr($hex9, 0, 2)],
            ['row' => '930', 'cell' => 1, 'value' => substr($hex2, 0, 2)], 
    ...
    public function calculate(int $value)
    {
        ...
        for ($i = 0; $i < 5; $i++) {
            $offset = $this->getOffset('920', 3, $i * 4);
