...
class Kia_Niro_Hibrid_millas_24c32_crc extends Script
{
    ...
    { $byte = $this->getByteForPosition('FA0', 0);
        
        $a = substr($byte, 0, 1);
        $b =substr($byte, -1);

        $byte2=$this->getByteForPosition('FA0', 1);
        $c = substr($byte2, 0, 1);
        $hex = $a . $b. $c ;
        $number = hexdec($hex); 
         
        return [
            'result' => round(($number*128)*0.621371),
    ...
    private function calculateRepeatedValue(int $value) : array
    {
        ...
        $result = round(($value/128)/0.621371);
        ...
        return
         [
            ...
            ['row' => 'FA0', 'cell' => 0, 'value' => substr($hex2, 0, 2)],
            ['row' => 'FA0', 'cell' => 1, 'value' => substr($hex9, 0, 2)], 
            ['row' => 'FA0', 'cell' => 4, 'value' => substr($hex2, 0, 2)],
            ['row' => 'FA0', 'cell' => 5, 'value' => substr($hex9, 0, 2)], 
            ['row' => 'FA0', 'cell' => 8, 'value' => substr($hex2, 0, 2)],
            ['row' => 'FA0', 'cell' => 9, 'value' => substr($hex9, 0, 2)], 
            ['row' => 'FA0', 'cell' => 12, 'value' => substr($hex2, 0, 2)],
            ['row' => 'FA0', 'cell' => 13, 'value' => substr($hex9, 0, 2)], 
            
            ['row' => 'FB0', 'cell' => 0, 'value' => substr($hex2, 0, 2)],
            ['row' => 'FB0', 'cell' => 1, 'value' => substr($hex9, 0, 2)], 
    ...
    public function calculate(int $value)
    {
        ...
        for ($i = 0; $i < 5; $i++) {
            $offset = $this->getOffset('FA0', 3, $i * 4);
