...
class Ford_Windstar_v2_HC912B32 extends Script
{
    public function getResult()
    {
        $byte = $this->getByteForPosition('240', 1);
        
        $a = substr($byte, 0, 1);
        $b =substr($byte, -1);

        $byte2=$this->getByteForPosition('240', 0);
        $c = substr($byte2, -1);
        $hex = $c . $a. $b ;
        $number = hexdec($hex); 
    ...
    public function calculate(int $value)
    {
        ...
        return [
            ...
            ['row' => '280', 'cell' => 0, 'value' => substr($hex, 0, 2)],
