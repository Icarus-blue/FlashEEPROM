<?php

namespace App\Scripts;

class Honda_Civic_Hybrid_93c76 extends Script
{
    public function getResult()
    {
        $hex = $this->getByteForPosition('180', 3) . $this->getByteForPosition('180', 2);
        $number = hexdec($hex);
         
        return [
            'result' => ((65535-$number)* 32),
            'image' => 'assets/honda.png',
            'texts' => [
                'Civic Hybrid ',
                'Eeprom 93c76',
                'www.FlashEeprom.com'
            ],
            'fileprefix' => 'flasheeprom'
        ];
    }
    
    public function calculate(int $value)

    {   
        $result = round(65535-($value/32));
        $hex = str_pad(dechex($result), 4, '0', STR_PAD_LEFT);
        $result2 = round(($value/32));
        $hex2 = str_pad(dechex($result2), 4, '0', STR_PAD_LEFT);

        return
         [

            ['row' => '180', 'cell' => 0, 'value' => substr($hex2, 2, 2)],
            ['row' => '180', 'cell' => 1, 'value' => substr($hex2, 0, 2)],
            ['row' => '180', 'cell' => 2, 'value' => substr($hex, 2, 2)],
            ['row' => '180', 'cell' => 3, 'value' => substr($hex, 0, 2)],
            ['row' => '180', 'cell' => 4, 'value' => substr($hex2, 2, 2)],
            ['row' => '180', 'cell' => 5, 'value' => substr($hex2, 0, 2)],
            ['row' => '180', 'cell' => 6, 'value' => substr($hex, 2, 2)],
            ['row' => '180', 'cell' => 7, 'value' => substr($hex, 0, 2)],
            ['row' => '180', 'cell' => 8, 'value' => substr($hex2, 2, 2)],
            ['row' => '180', 'cell' => 9, 'value' => substr($hex2, 0, 2)],
            ['row' => '180', 'cell' => 10, 'value' => substr($hex, 2, 2)],
            ['row' => '180', 'cell' => 11, 'value' => substr($hex, 0, 2)],
            ['row' => '180', 'cell' => 12, 'value' => substr($hex2, 2, 2)],
            ['row' => '180', 'cell' => 13, 'value' => substr($hex2, 0, 2)],
            ['row' => '180', 'cell' => 14, 'value' => substr($hex, 2, 2)],
            ['row' => '180', 'cell' => 15, 'value' => substr($hex, 0, 2)],

            ['row' => '190', 'cell' => 0, 'value' => substr($hex2, 2, 2)],
            ['row' => '190', 'cell' => 1, 'value' => substr($hex2, 0, 2)],
            ['row' => '190', 'cell' => 2, 'value' => substr($hex, 2, 2)],
            ['row' => '190', 'cell' => 3, 'value' => substr($hex, 0, 2)],
            ['row' => '190', 'cell' => 4, 'value' => substr($hex2, 2, 2)],
            ['row' => '190', 'cell' => 5, 'value' => substr($hex2, 0, 2)],
            ['row' => '190', 'cell' => 6, 'value' => substr($hex, 2, 2)],
            ['row' => '190', 'cell' => 7, 'value' => substr($hex, 0, 2)],
            ['row' => '190', 'cell' => 8, 'value' => substr($hex2, 2, 2)],
            ['row' => '190', 'cell' => 9, 'value' => substr($hex2, 0, 2)],
            ['row' => '190', 'cell' => 10, 'value' => substr($hex, 2, 2)],
            ['row' => '190', 'cell' => 11, 'value' => substr($hex, 0, 2)],
            ['row' => '190', 'cell' => 12, 'value' => substr($hex2, 2, 2)],
            ['row' => '190', 'cell' => 13, 'value' => substr($hex2, 0, 2)],
            ['row' => '190', 'cell' => 14, 'value' => substr($hex, 2, 2)],
            ['row' => '190', 'cell' => 15, 'value' => substr($hex, 0, 2)],

            ['row' => '1A0', 'cell' => 0, 'value' => substr($hex2, 2, 2)],
            ['row' => '1A0', 'cell' => 1, 'value' => substr($hex2, 0, 2)],
            ['row' => '1A0', 'cell' => 2, 'value' => substr($hex, 2, 2)],
            ['row' => '1A0', 'cell' => 3, 'value' => substr($hex, 0, 2)],
            ['row' => '1A0', 'cell' => 4, 'value' => substr($hex2, 2, 2)],
            ['row' => '1A0', 'cell' => 5, 'value' => substr($hex2, 0, 2)],
            ['row' => '1A0', 'cell' => 6, 'value' => substr($hex, 2, 2)],
            ['row' => '1A0', 'cell' => 7, 'value' => substr($hex, 0, 2)],
            ['row' => '1A0', 'cell' => 8, 'value' => substr($hex2, 2, 2)],
            ['row' => '1A0', 'cell' => 9, 'value' => substr($hex2, 0, 2)],
            ['row' => '1A0', 'cell' => 10, 'value' => substr($hex, 2, 2)],
            ['row' => '1A0', 'cell' => 11, 'value' => substr($hex, 0, 2)],
            ['row' => '1A0', 'cell' => 12, 'value' => substr($hex2, 2, 2)],
            ['row' => '1A0', 'cell' => 13, 'value' => substr($hex2, 0, 2)],
            ['row' => '1A0', 'cell' => 14, 'value' => substr($hex, 2, 2)],
            ['row' => '1A0', 'cell' => 15, 'value' => substr($hex, 0, 2)],

            ['row' => '1B0', 'cell' => 0, 'value' => substr($hex2, 2, 2)],
            ['row' => '1B0', 'cell' => 1, 'value' => substr($hex2, 0, 2)],
            ['row' => '1B0', 'cell' => 2, 'value' => substr($hex, 2, 2)],
            ['row' => '1B0', 'cell' => 3, 'value' => substr($hex, 0, 2)],
            ['row' => '1B0', 'cell' => 4, 'value' => substr($hex2, 2, 2)],
            ['row' => '1B0', 'cell' => 5, 'value' => substr($hex2, 0, 2)],
            ['row' => '1B0', 'cell' => 6, 'value' => substr($hex, 2, 2)],
            ['row' => '1B0', 'cell' => 7, 'value' => substr($hex, 0, 2)],
            ['row' => '1B0', 'cell' => 8, 'value' => substr($hex2, 2, 2)],
            ['row' => '1B0', 'cell' => 9, 'value' => substr($hex2, 0, 2)],
            ['row' => '1B0', 'cell' => 10, 'value' => substr($hex, 2, 2)],
            ['row' => '1B0', 'cell' => 11, 'value' => substr($hex, 0, 2)],
            ['row' => '1B0', 'cell' => 12, 'value' => substr($hex2, 2, 2)],
            ['row' => '1B0', 'cell' => 13, 'value' => substr($hex2, 0, 2)],
            ['row' => '1B0', 'cell' => 14, 'value' => substr($hex, 2, 2)],
            ['row' => '1B0', 'cell' => 15, 'value' => substr($hex, 0, 2)],

        

        


        ];
    }
}
