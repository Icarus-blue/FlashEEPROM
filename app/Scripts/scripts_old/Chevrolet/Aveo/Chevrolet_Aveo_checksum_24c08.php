...
class Chevrolet_Aveo_checksum_24c08 extends Script
{
    ...
    {
        $hex = $this->getByteForPosition('340', 4);
        $hex .= $this->getByteForPosition('340', 5);
        
        $base = intdiv(hexdec($hex) * 8, 128);
        
        $map = array_flip($this->primary_map);
        $primary_base = $this->getByteForPosition('340', 7);
        $portion = $map[$primary_base];
        
        $units = 0;
        for ($i = 1; $i < 32; $i++) {
            $byte = $this->getByteFromOffset('340', 7, $i * 4);
    ...
    private function calculateRepeatedValue(int $value) : array
    {
        ...
        for ($i = 0; $i < 32; $i++) {
            $offset = $this->getOffset('340', 4, $i * 4);
