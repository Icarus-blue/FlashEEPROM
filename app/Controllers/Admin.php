...
class Admin extends BaseController
{
    ...
    {
        if (!$this->isUserLoggedIn() || !($this->user->canManageAllUsers() || $this->user->isModerator())   ) {
    ...
    public function smtgenerator_admin()
    {
        ...
        // dd($this->request->getMethod() == 'get');
        if ($this->request->getMethod() == 'get') {
            $keylist = $this->registerkey->getKeys();
            $this->renderDefaultLayout('key_register', [
                'downloads' => $this->downloadModel->getTotalDownloads(),
                'users' => $items,
                'pager' => $this->userModel->pager,
                'keylist' => $keylist,
            ...
            $keylist = $this->registerkey->createKey($data);
            $this->renderDefaultLayout('key_register', [
                'keylist' => $keylist,
                'pager' => $this->userModel->pager,
                'tableOnly' => false,
    ...
    {
        $keylist = $this->registerkey->getKeyById($id);
        $data = [
            "status" => 1,
        ];
    ...
    public function delete_secretkey($id)
    {
        if (!$this->isUserLoggedIn() || !$this->user->canManageAllUsers()) {
            return $this->forbidden();
        }
